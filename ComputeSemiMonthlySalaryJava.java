/*
 * Click nbfs://nbhost/SystemFileSystem/Templates/Licenses/license-default.txt to change this license
 * Click nbfs://nbhost/SystemFileSystem/Templates/Classes/Main.java to edit this template
 */
package computesemimonthlysalary.java;

/**
 *
 * @author PGN-0253
 */
public class ComputeSemiMonthlySalaryJava {
    
 public static void main(String[] args) {

        // Array for Employee Names
        String[] employeeNames = {
            "Manuel Garcia", "Lim Antonio", "Aquino Bianca", "Reyes Isabella", "Hernandez Eduard",
            "Villanueva Andrea Mae", "San Jose Brad", "Romualdez Alice", "Atienza Rosie", "Alvaro",
            "Salcedo", "Lopez", "Farala", "Martinez", "Romualdez", "Mata", "De Leon", "San Jose",
            "Rosario", "Bautista", "Lazaro", "Delo Santos", "Santos", "Del Rosario", "Tolentino",
            "Gutierrez", "Manalaysay", "Villegas", "Ramos", "Maceda", "Aguilar", "Castro", "Martinez", "Santos"
        };

        double[] hoursWorked = {
            80,70,80,90,70,70,80,70,80,
            80,80,70,80,80,80,70,70,80,
            80,70,80,80,80,85,75,
            80,75,80,85,75,80,80,75,70
        };

        double[] hourlyRate = {
            535,357,357,313,313,255,133,133,313,
            302,229,142,318,255,249,133,133,138,
            138,142,133,133,142,147,147,
            142,133,133,133,313,313,313,313,313
        };

        System.out.println("MotorPH Semi-Monthly Salary Computation");
        System.out.println("--------------------------------------");

        // Loop through employees
        for (int i = 0; i < employeeNames.length; i++) {

            if(hoursWorked[i] <= 0 || hourlyRate[i] <= 0) {
                System.out.println("Invalid data for employee: " + employeeNames[i]);
                continue;
            }

            double semiMonthlySalary = hoursWorked[i] * hourlyRate[i];

            // Display results
            System.out.println("Employee Name: " + employeeNames[i]);
            System.out.println("Total Hours Worked: " + hoursWorked[i]);
            System.out.println("Hourly Rate: " + hourlyRate[i]);
            System.out.println("Semi-Monthly Salary: " + semiMonthlySalary);
            System.out.println("--------------------------------------");
        }

        // Verification Message
        System.out.println("Computation verified successfully!");
         }
        
       
         }
        
    
            
            
                    
                    
            
            
            
            

            
            
    

